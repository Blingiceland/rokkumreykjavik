...
export const metadata: Metadata = { title: "Plaköt", robots: { index: false, follow: false } };
...
export default function ArtistPage({ searchParams }: { searchParams: { villa?: string } }) {
  if (isAdmin()) return <AdminHub logout={logout} />;

  return (
    <main className="grid min-h-screen place-items-center px-4">
      <form action={login} className="w-full max-w-sm">
        <p className="font-mono text-[11px] uppercase tracking-[0.3em] text-neon">Rokk í Reykjavík · plaköt</p>
