...
import { ThuleTakeover } from "@/components/thule/thule-takeover";

export const dynamic = "force-dynamic";
export const metadata: Metadata = { title: "Thule takeover", robots: { index: false, follow: false } };
...
async function login(formData: FormData) {
  ...
  redirect("/thule?villa=1");
}

export default function ThulePage({ searchParams }: { searchParams: { villa?: string } }) {
  if (isAdmin()) return <ThuleTakeover />;

  return (
    <main className="grid min-h-screen place-items-center px-4">
      <form action={login} className="w-full max-w-sm">
        <p className="font-mono text-[11px] uppercase tracking-[0.3em] text-neon">Thule · takeover (drög)</p>
        ...
        <p className="mt-2 font-mono text-[11px] leading-relaxed text-bone-faint">
          Lokuð forskoðun. Sláðu inn lykilorð til að opna Thule-útgáfuna.
        </p>
        <input
          type="password"
          name="password"
