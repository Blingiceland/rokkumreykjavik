...
export const metadata: Metadata = { title: "Thule · plaköt", robots: { index: false, follow: false } };
...
async function login(formData: FormData) {
  ...
  if (pw === ADMIN_PASSWORD) {
    ...
    redirect("/thule/plakot");
  }
  redirect("/thule/plakot?villa=1");
}
...
export default function ThulePosterHubPage({ searchParams }: { searchParams: { villa?: string } }) {
  // The whole hub renders inside a Thule look so the chrome matches the takeover;
      ...
      <main className="grid min-h-screen place-items-center px-4">
        <form action={login} className="w-full max-w-sm">
          ...
          <p className="mt-2 font-mono text-[11px] leading-relaxed text-bone-faint">
            Lokuð forskoðun. Sláðu inn lykilorð til að opna plakat-miðstöðina.
          </p>
          <input
            type="password"
            name="password"
